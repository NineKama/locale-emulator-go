...
5. For default x86 launches, it installs native stubs generated by Go without loading a Go runtime into the target. x64 and explicit DLL overrides load the shared engine. Remote API addresses are resolved by module and RVA rather than assuming identical ASLR addresses.
6. It checks native hook installation or calls `InstallLocale` for the DLL engine, then resumes the main thread. Failures terminate the newly created process so partial installation cannot leave it suspended.
...
The default x86 engine uses generated stdcall machine code and Windows APIs directly. Temporary conversion buffers are allocated from the Windows process heap and freed after each call. The following shared-library details apply to x64 and explicit DLL overrides.
...
## x86 address-space overhead

The default x86 path avoids this overhead by installing native stubs without a Go DLL. The retained c-shared DLL, used for explicit engine overrides, brings a Go runtime into the target. A small physical
working set does not imply a small virtual address reservation. In a synthetic
non-Large-Address-Aware x86 process, loading the current Go engine without hooks
reduced free address space by about 530 MiB. Go's 32-bit runtime attempts an initial
heap reservation of 512 MiB (with smaller fallback sizes). This can affect native
games that need large bitmap allocations, even when the computer has free RAM.

Run `scripts/memory-probe.ps1 -CC32 <x86-compiler>` to compare baseline, DLL-loaded,
and hooked states. It operates on a generated test executable only. Values depend
on the toolchain and address layout; this is a diagnostic, not a compatibility test
or proof of the cause of every allocation failure. Reducing this overhead requires
an engine design change; no game executable flags are patched by the launcher.
